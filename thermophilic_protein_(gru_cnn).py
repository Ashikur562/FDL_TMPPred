# -*- coding: utf-8 -*-
"""Thermophilic_Protein_(GRU_CNN).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1raUHE0NMt-B5lZuU8XUf-xexV-Md9fp5
"""
...
file_name = r'/kaggle/working/model-6-result.csv'
validation_file = r'/kaggle/working/model-6-validation.csv'
...
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import GRU, MaxPooling1D, BatchNormalization, Conv1D, Flatten, Dense
from tensorflow.keras.optimizers import Adam

def build_model(input_shape, gru_units=8, filters=32, kernel_size=3, pool_size=2, learning_rate=1e-2):
    """
    Function to define a GRU-CNN model for sequence data.
    ...
    # GRU Layer with reduced units and added dropout
    model.add(GRU(gru_units, input_shape=input_shape, return_sequences=True))
...
for train_index, val_index in kf.split(xtrain_cnn):
    ...
    validation_metrics = func(model, f"APAAC_Model6_Fold_{fold_no}", X_val, y_val)
...
test_metrics = total_matric_cal(ytest_cnn, y_pred_classes, pred, "APAAC_Model6_Test_Set")
print("\nTest Set Metrics:")
print(test_metrics)
...
for train_index, val_index in kf.split(xtrain_cnn):
    ...
    # Evaluate on validation fold using func()
    validation_metrics = func(model, f"CTDC_Model6_Fold_{fold_no}", X_val, y_val)
    all_validation_metrics = pd.concat([all_validation_metrics, validation_metrics], ignore_index=True)
...
test_metrics = total_matric_cal(ytest_cnn, y_pred_classes, pred, "CTDC_Model6_Test_Set")
print("\nTest Set Metrics:")
print(test_metrics)
...
for train_index, val_index in kf.split(xtrain_cnn):
    ...
    # Evaluate on validation fold using func()
    validation_metrics = func(model, f"DDE_Model6_Fold_{fold_no}", X_val, y_val)
    all_validation_metrics = pd.concat([all_validation_metrics, validation_metrics], ignore_index=True)
...
test_metrics = total_matric_cal(ytest_cnn, y_pred_classes, pred, "DDE_Model6_Test_Set")
print("\nTest Set Metrics:")
print(test_metrics)

"""# FF"""

df = pd.read_csv('/content/FF.csv')
columns = df.columns.tolist()
# Filter the columns to remove data we do not want
columns = [c for c in columns if c not in ["Target"]]
# Store the variable we are predicting
...
for train_index, val_index in kf.split(xtrain_cnn):
    ...
    # Evaluate on validation fold using func()
    validation_metrics = func(model, f"DistancePair_Model6_Fold_{fold_no}", X_val, y_val)
    all_validation_metrics = pd.concat([all_validation_metrics, validation_metrics], ignore_index=True)
...
test_metrics = total_matric_cal(ytest_cnn, y_pred_classes, pred, "DistancePair_Model6_Test_Set")
print("\nTest Set Metrics:")
print(test_metrics)